...
export function EmailIcon({ className = 'w-5 h-5' }: IconProps) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
      ...
      <path strokeLinecap="round" strokeLinejoin="round" d="M3 6.75l9 6.75 9-6.75" />
    </svg>
  );
}
...
export const platformIconMap: Record<string, (props: IconProps) => JSX.Element> = {
...
export function PlatformIcon({ platform, className }: { platform: string; className?: string }) {
  const Icon = platformIconMap[platform] || LinkIcon;
