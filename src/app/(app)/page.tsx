...
import { fetchPageData, fetchWorkExperience } from '@/lib/payload';

// Used only if the Work Experience collection is empty (e.g. DB not seeded yet)
...
export default async function Home() {
  try {
    const [{ profile, socialLinks, ctaButtons, projects, blogPosts }, workExperience] = await Promise.all([
      fetchPageData(),
      fetchWorkExperience(),
    ]);
    const experiences = workExperience.length > 0 ? workExperience : fallbackExperiences;
    ...
    return (
      <>
        ...
        <WorkExperienceSidebar experiences={experiences} />
